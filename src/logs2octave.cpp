...
int main(int argc, char ** argv){
    ...
    if(log_type == CRPARAMS){
        ...
        while(fread((struct timeval*)&log_time, sizeof(struct timeval), 1, file_in)){
            ...
            fprintf(file_out, "%sCRTS_rx_t(%i) = %li + 1e-6*%li;\n",  node_prefix, i, log_time.tv_sec, log_time.tv_usec);
            fprintf(file_out, "%sCRTS_rx_bytes(%i) = %i;\n\n",  node_prefix, i, bytes);
            i++;
        }    
    }
