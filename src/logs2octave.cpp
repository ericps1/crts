...
int main(int argc, char **argv) {
  ...
  case PHY_RX_LOG: {
    ...
    if (totalNumReps > 1) {
      fprintf(file_out, "phy_rx_t_all_repetitions{%u} = phy_rx_t;\n",
              repNumber);
      // metrics
      fprintf(file_out, "phy_rx_Control_valid_all_repetitions{%u}     = "
                        "phy_rx_Control_valid;\n",
              repNumber);
      fprintf(file_out, "phy_rx_Payload_valid_all_repetitions{%u}     = "
                        "phy_rx_Payload_valid;\n",
              repNumber);
      fprintf(file_out, "phy_rx_EVM_all_repetitions{%u}   = phy_rx_EVM;\n",
              repNumber);
      fprintf(file_out, "phy_rx_RSSI_all_repetitions{%u}  = phy_rx_RSSI;\n",
              repNumber);
      fprintf(file_out, "phy_rx_CFO_all_repetitions{%u}   = phy_rx_CFO;\n",
              repNumber);
      fprintf(file_out,
      ...
      fprintf(file_out,
              ...
              repNumber);
      fprintf(file_out, "phy_rx_BPS_all_repetitions{%u}   = phy_rx_BPS;\n",
              repNumber);
      fprintf(file_out, "phy_rx_fec0_all_repetitions{%u}  = phy_rx_fec0;\n",
              repNumber);
      fprintf(file_out, "phy_rx_fec1_all_repetitions{%u}  = phy_rx_fec1;\n",
              repNumber);
      // parameters
      fprintf(file_out, "phy_rx_numSubcarriers_all_repetitions{%u}    = "
                        "phy_rx_numSubcarriers;\n",
              repNumber);
      fprintf(file_out,
      ...
      fprintf(file_out,
              ...
              repNumber);
      fprintf(file_out, "phy_rx_freq_all_repetitions{%u}  = phy_rx_freq;\n",
              repNumber);
      fprintf(file_out, "phy_rx_rate_all_repetitions{%u}  = phy_rx_rate;\n",
              repNumber);
  ...
  case PHY_TX_LOG: {
    ...
    // If appending to the multiFile, then put the data into the next elements
    // of the cell arrays
    if (totalNumReps > 1) {
      fprintf(file_out, "phy_tx_t_all_repetitions{%u} = phy_tx_t;\n",
              repNumber);
      fprintf(file_out, "phy_tx_numSubcarriers_all_repetitions{%u} = "
                        "phy_tx_numSubcarriers;\n",
              repNumber);
      fprintf(file_out, "phy_tx_cp_len_all_repetitions{%u} = phy_tx_cp_len;\n",
              repNumber);
      fprintf(file_out,
      ...
      fprintf(file_out,
              ...
              repNumber);
      fprintf(file_out, "phy_tx_freq_all_repetitions{%u} = phy_tx_freq;\n",
              repNumber);
      fprintf(file_out,
      ...
      fprintf(file_out,
              ...
              repNumber);
      fprintf(file_out, "phy_tx_rate_all_repetitions{%u} = phy_tx_rate;\n",
              repNumber);
      fprintf(file_out,
              "phy_tx_mod_scheme_all_repetitions{%u} = phy_tx_mod_scheme;\n",
              repNumber);
      fprintf(file_out, "phy_tx_fec0_all_repetitions{%u} = phy_tx_fec0;\n",
              repNumber);
      fprintf(file_out, "phy_tx_fec1_all_repetitions{%u} = phy_tx_fec1;\n\n",
              repNumber);
  ...
  case NET_RX_LOG: {
    ...
    // then put the data into the next elements of the cell arrays
    if (totalNumReps > 1) {
      fprintf(file_out, "net_rx_t_all_repetitions{%u} = net_rx_t;\n",
              repNumber);
      fprintf(file_out, "net_rx_bytes_all_repetitions{%u} = net_rx_bytes;\n\n",
              repNumber);
      fprintf(file_out,
  ...
  case NET_TX_LOG: {
    ...
    // then put the data into the next elements of the cell arrays
    if (totalNumReps > 1) {
      fprintf(file_out, "net_tx_t_all_repetitions{%u} = net_tx_t;\n",
              repNumber);
      fprintf(file_out, "net_tx_bytes_all_repetitions{%u} = net_tx_bytes;\n\n",
              repNumber);
      fprintf(file_out,
