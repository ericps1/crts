...
figure;
stem(ECR_rx_t,ECR_rx_Header_valid);
title('Valid Headers');
xlabel('Time (s)');
ylabel('Valid Header');
ylim([-1 2]);
...
figure;
stem(ECR_rx_t,ECR_rx_Payload_valid);
...
figure;
plot(ECR_rx_t,ECR_rx_BPS);
title('Bits Per Symbol');
xlabel('Time (s)');
ylabel('Bits Per Symbol');
ylim([0, max(ECR_rx_BPS)+1]);

figure;
plot(ECR_rx_t,ECR_rx_fec0);
