...
void SC_BER_Sweep::execute(int node, char fb_type, void *_arg) {
  ...
  switch (fb_type) {
    case CRTS_RX_STATS:
      ...
      fprintf(log, "BER(%i,%i) = %f;\n", node+1, data_ind, rx_stats.avg_ber);
      fprintf(log, "PER(%i,%i) = %f;\n", node+1, data_ind, rx_stats.avg_per);
      fprintf(log, "Throughput(%i,%i) = %f;\n", node+1, data_ind, rx_stats.avg_throughput);
      fprintf(log, "EVM(%i,%i) = %f;\n", node+1, data_ind, rx_stats.avg_evm);
      fprintf(log, "RSSI(%i,%i) = %f;\n", node+1, data_ind, rx_stats.avg_rssi);
      fprintf(log, "\n");
      fclose(log);
      break;
  }
