    ...
    if (SOCK_RAW == socketType)
    {
        switch (raw_protocol)
        {
            case RAW:
                ...
                break;
            case UDP:
                TRACE("setting raw sock proto udp\n");
...
{    
 ...
 // Join/Leave group with IGMPv3 SSM support.
 // Supported on Linux and MacOSX 10.7 and later
                            ...
                            const char*         interfaceName)
{
    ...
    if  (ProtoAddress::IPv6 == groupAddress.GetType())
    {
        PLOG(PL_ERROR, "ProtoSocket::JoinGroup() error: IPv6 not supported\n");
        return false;
    } 
    else
#endif // HAVE_IPV6
    {
        ...
        if ( result  < 0 ) {
               PLOG(PL_ERROR, "ProtoSocket::JoinGroup() failed : %s\n", strerror( errno ));
               return false;
        }
		return true;
    }
...
{    
    ...
    if (ProtoAddress::IPv6 == groupAddress.GetType())
    {
        PLOG(PL_ERROR, "ProtoSocket::LeaveGroup() error: IPv6 not supported\n");
        return false;
    }
    else
#endif //HAVE_IPV6
...
#else
#ifdef RES_USE_INET6
    /* not sure this is needed anymore? (Since we use getaddrinfo()?)
