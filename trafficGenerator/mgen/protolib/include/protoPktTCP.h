...
class ProtoPktTCP : public ProtoPkt
{
    public:
        ...
        UINT16 GetFlags() const
            {return (0x01ff & GetUINT16(OFFSET_FLAGS));}
        ...
        UINT16 GetUrgentPointer() const
            {return GetUINT16(OFFSET_URGENT);}
        ...
        {
            UINT16 field = 0xfe00 & GetUINT16(OFFSET_FLAGS);
            SetUINT16(OFFSET_FLAGS, field | flags);
        }
        void ClearFlags()
        {
            UINT16 field = 0xfe00 & GetUINT16(OFFSET_FLAGS);
            SetUINT16(OFFSET_FLAGS, field);
        } 
        void SetFlag(Flag flag)
        {
            UINT16 field = GetUINT16(OFFSET_FLAGS);
            SetUINT16(OFFSET_FLAGS, field | (UINT16)flag);
        }       
        ...
        void SetWindowSize(UINT16 windowSize)
            {SetUINT16(OFFSET_WINDOW, windowSize);}
        void SetChecksum(UINT16 checksum)
            {SetUINT16(OFFSET_CHECKSUM, checksum);}
        void SetUrgentPointer(UINT16 value)
        {
            SetFlag(FLAG_URG);
            SetUINT16(OFFSET_URGENT, value);
        }
