...
void RaprApp::OnStopEvent(char* buffer, int len)
{
    rapr.OnCommand(Rapr::STOPEVENT,buffer,BehaviorEvent::MGEN_EVENT,false);
...
void RaprApp::OnMsgReceive(MgenMsg& msg)
{
    ...
    {
      char msgBuffer[512];
      sprintf(msgBuffer,"type>mgenMsgRecv action>logicID payloadLogicID>%d msgSrc>%s/%hu",raprPayload.GetLogicID(),msg.GetSrcAddr().GetHostString(),msg.GetSrcAddr().GetPort());
  ...
  if (rapr_control_remote)
    {
      ...
      sprintf(msgBuffer,"type>Application action>sendCtrlPipeCmd cmd>\"%s\"",buffer);
      rapr.LogEvent("RAPR",msgBuffer);
	  ...
	  sprintf(msgBuffer,"type>Error action>sendCtrlPipeCmd cmd>\"%s\"",buffer);
	  rapr.LogEvent("RAPR",msgBuffer);
    ...
    else if (!strncmp("help",cmd,len) || strncmp("--help",cmd,len) || strncmp("-h",cmd,len) )
    {
        fprintf(stderr,"%s:version %s\n",filename,RAPR_VERSION);
        Usage();
        return false;
  ...
  if (ProtoSocket::RECV == theEvent)
    {
	  ...
	  char msgBuffer [512];
	  sprintf(msgBuffer,"type>Application action>rti_event cmd>%s arg>\"%s\"",cmd,arg);
	  rapr.LogEvent("RAPR",msgBuffer);
	  ...
	  if (!OnCommand(cmd,arg,BehaviorEvent::RTI_EVENT)) 
	    {
	      ...
	      sprintf(msgBuffer,"type>error eventSource>rti_event cmd>\"%s\" arg>\"%s\"",cmd,arg);
	      rapr.LogEvent("RAPR",msgBuffer);
            } 
	}
      else
	{
	  ...
	  rapr.LogEvent("RAPR","type>error action>rti_event info>\"Error reading from pipe\"");
	}
    }
  else
    {
      rapr.LogEvent("RAPR","type>error action>ctrlEvent info>\"Unhandled event\"");
    }
