...
int main (int argc, char *argv[])
{
  ...
  FFTcoeffs = new double[lda*nstreams];                      /* FFT coeffs */
  means = new double[lda*nstreams];                      /* average coeffs */
  streams = new Sprng*[nstreams];                    /*random number streams*/
...
void FFTCalc(int nstreams, int nruns, int n) 
{
  ...
  for(k=0; k<nruns; k++)	/* Repeat calculation 'nruns' times         */
  {
    for (j=0; j<nstreams; j++)  /*  fill array with random numbers          */
      for (i=0; i<n; i++) 
	FFTcoeffs[i+j*lda] = streams[j]->sprng();
