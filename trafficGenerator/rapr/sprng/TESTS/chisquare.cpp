...
double chiF (double chiSq);
double chisquare (long *actual,double *probability,long n,long k, int *nb);
double chipercent (double chiSq, long fr);
double gammp (double a, double x);
double KSpercent (double value, long n);
double KS (double *V, long n, double (*F)(double));

void  gser (double *gamser, double a, double x, double *gln);
void  gcf (double *gammcf, double a, double x, double *gln);
double gammln (double xx);

void  chiGammaClearErrMess (void);
const char  *chiGammaReadErrMess (void);
void  chiGammaFlagError (const char *errorText);
...
const char *chiGammaReadErrMess(void)  /* returns the error message     */
{                                /* from the buffer               */
  return (errorMessage);
}

void chiGammaFlagError(const char *errorText) /* writes to the error message   */
{                                       /* buffer                        */
