...
{
  int in, i, n, my_n, in_old, n_old, nprocs, myid, temp;
...
int initialize_function(int * n, int * in_old, int * n_old, char *filename)
{
  ...
  else				/* continue from previously stored state   */
  {
	...
	exit(1);
      ...
      i = fread(in_old,1,sizeof(int),fp); /* cumulative # in circle previously   */
      i = fread(n_old,1,sizeof(int),fp);  /* cumulative # of samples previously  */
      i = fread(&old_nprocs,1,sizeof(int),fp);/*cumulative # of previous samples */

      if(old_nprocs != nprocs)
	fprintf(stderr,"Number of processes different in current and previous\
 runs. Strange thing can happen ... \n");
      
      i = fread(&size,1,sizeof(int),fp);  /* size of stored stream state       */

      i = fread(buffer,1,size,fp);	/* copy stream state to buffer             */
